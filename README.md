...
- 回测和实际生成共用固定首组及组合覆盖逻辑；单策略回测会显示同窗口、同组数的随机基线；设置随机种子时两者使用同一种子，仅用于同条件对照。
...
- 如果本地最新开奖日期早于按开奖规则推算的最近开奖日，会提示先刷新历史数据。
- `--seed 9` 固定随机种子，便于复现。
- 终端会提示下一期开奖预计时间。
...
- 操作区：开始分析/预测、刷新历史数据、停止执行、清空日志、导出结果、保存当前参数方案、加载参数方案。
